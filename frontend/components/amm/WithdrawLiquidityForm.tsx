...
export function WithdrawLiquidityForm({
  ammId,
  poolAddress,
  mintA,
  mintB,
}: WithdrawLiquidityFormProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [accountAInfo, accountBInfo, liquidityMintInfo, userLPInfo] =
          await Promise.all([
        ...
        if (lpAmount > 0 && totalSupply > 0) {
          const shareA = Math.floor(
            (lpAmount * poolReserveA) / (totalSupply + MINIMUM_LIQUIDITY)
          );
          const shareB = Math.floor(
            (lpAmount * poolReserveB) / (totalSupply + MINIMUM_LIQUIDITY)
          );
      ...
      <Stack gap="md">
        ...
        {estimatedTokenA > 0 && estimatedTokenB > 0 && (
          <Stack gap="xs">
            ...
            <Text size="sm">
              Token A: <strong>{estimatedTokenA.toLocaleString()}</strong>
            </Text>
            <Text size="sm">
              Token B: <strong>{estimatedTokenB.toLocaleString()}</strong>
            </Text>
          </Stack>
        )}
        ...
        <Text size="xs" c="dimmed">
          Note: LP tokens will be burned and you'll receive proportional amounts
          of both tokens.
        </Text>
      </Stack>
    </Card>
